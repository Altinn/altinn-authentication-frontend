...
public class SystemRegisterControllerTest : IClassFixture<CustomWebApplicationFactory<SystemRegisterController>>
{
    ...
    [Fact]
    public async Task GetSystemTypeList_ReturnsListOK() 
    {        
        ...
        List<RegisteredSystemDTO>? list = JsonConvert.DeserializeObject<List<RegisteredSystemDTO>>(await response.Content.ReadAsStringAsync());
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.True(list is not null && list.Count > 0);        
        Assert.True(list[0].Id is not null);
        Assert.True(list[0].Navn is not null);
        Assert.True(list[0].Beskrivelse is not null);
    }
}
